...
; (function (factory) {
    ...
    return function (config) {
        ...
        this.upcheck = function (md5) {
            ...
            //}
            if (that.apped_data && that.apped_data.length > 0) {
                //that.apped_data.forEach(v => {
                //    this.form_data.delete(v.key);
                //})
                for (let v of that.apped_data) {
                    this.form_data.delete(v.key);
                }
            }
            xhrs = null;
        ...
        this.upload = function (file) {
            ...
            if (this.min_size) {
                ...
                if (file.size < limitNum) {
                    ...
                    console.log(file)
                    this.upload_single(file, this.single_url)
                    return;
                }
            }
            if (this.max_size) {
        ...
        this.upload_single = function (file, uploadUrl) {
            if (!file) {
                return;
            }
            ...
            if (this.apped_data && this.apped_data.length > 0) {
                this.apped_data.forEach(v => {
                    singleFormData.append(v.key, v.value);
                })
            }
            singleFormData.append('file', file);
